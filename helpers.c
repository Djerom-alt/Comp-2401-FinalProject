...
void rooms_connect(struct Room* a, struct Room* b){
    // edge case
    if(a->connection_count > MAX_CONNECTIONS || b->connection_count > MAX_CONNECTIONS) return;
    // bi directional connection
    a->adjacent_rooms[a->connection_count++] = b;
    b->adjacent_rooms[b->connection_count++] = a;
} 

// start simulation function
void start_simulation(struct House* house) {

    // // bool collector
    // bool all_exited=false;
    // while(!all_exited) {
        
    //     all_exited = true;   // reset at start of loop

    //     if(!house->ghost.has_exited) {
    //         // to exit simulation, all hunters and ghost must have exited
    //         ghost_thread(&(house->ghost));
    //     }
        
    //     all_exited&=house->ghost.has_exited;

    //     // loop through the hunter collection, each hunter turn their turn
    //     for(int i =0;i<house->hunter_collection.size;i++){
    //         if(!house->hunter_collection.array[i].has_exited){
    //             hunter_thread(house->hunter_collection.array+i);
    //         }
    //         all_exited&=house->hunter_collection.array[i].has_exited;
    //     }
    ...
    // ghost thread
    pthread_t ghost_t;
    pthread_create(&ghost_t, NULL, ghost_thread, &(house->ghost));

    pthread_t hunter_ts[house->hunter_collection.size];
    for(int i =0;i<house->hunter_collection.size;i++){
        pthread_create(&hunter_ts[i], NULL, hunter_thread, house->hunter_collection.array+i);
    }

    pthread_join(ghost_t, NULL);
    for(int i =0;i<house->hunter_collection.size;i++){
        pthread_join(hunter_ts[i], NULL);
    }
